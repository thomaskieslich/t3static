...
### Misc Tests ###
 composer() {
     ...
     command composer validate --working-dir "${FULL_PACKAGE_PATH}" --with-dependencies --strict || local EXIT_CODE=$?
...
json() {
    ...
    find "${FULL_PACKAGE_PATH}" ! -path "*/node_modules/*" -name "*.json" -print0 |
        xargs --null -r php "${TEST_PATH}/vendor/bin/jsonlint" || local EXIT_CODE=$?
...
md() {
    ...
    npx --prefix "${TEST_PATH}" markdownlint-cli2 "${FULL_PACKAGE_PATH}/**/*.md" \
        --config "${CONFIGURATION_PATH}/.markdownlint-cli2.yaml" || local EXIT_CODE=$?
    echoTestFooter "Markdown is Linted " ${EXIT_CODE}
    return ${EXIT_CODE}
}
...
yaml() {
    ...
    find "${FULL_PACKAGE_PATH}" ! -path "*/node_modules/*" -name "*.y*ml" -print0 |
        xargs --null -r php "${TEST_PATH}/vendor/bin/yaml-lint" --ansi || local EXIT_CODE=$?
